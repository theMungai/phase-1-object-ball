...
function numPointsScored(playerName){
    ...
    for(let key in gameInfo){
        if(gameInfo[key].players[playerName]){
            return gameInfo[key].players[playerName].points
        }
    }
}
...
function shoeSize(playerName){
    ...
    for(let key in gameInfo){
        if(gameInfo[key].players[playerName]){
            return gameInfo[key].players[playerName].shoe
        }
    }
}
...
function teamColors(team){
    ...
    for(let key in gameInfo){
        if(gameInfo[key].teamName === team){
            return gameInfo[key].colors
        }
    }
}
...
function playerNumbers(teamName){
    ...
    for(let key in gameInfo){
        if(gameInfo[key].teamName === teamName){
            return Object.values(gameInfo[key].players).map(player => player.number);
        }
    }
}
...
function playerStats(playerName){
    ...
    for(let key in gameInfo){
        if(gameInfo[key].players[playerName]){
            return gameInfo[key].players[playerName]
        }
    }
}
    ...
    for (let team in game) {
      for (let player in game[team].players) {
        if (game[team].players[player].shoe > maxShoeSize) {
          ...
          teamWithBiggestShoe = team;
        }
      }
    }
    
    return game[teamWithBiggestShoe].players[playerWithBiggestShoe].rebounds;
  }
